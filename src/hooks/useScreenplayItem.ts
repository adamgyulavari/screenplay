...
export const useScreenplayItem = (screenplay: DialogueItem[], index: number): DialogueItem | undefined => {
  return useMemo(() => {
...
export const useScreenplayItems = (screenplay: DialogueItem[], indexes: number[]): DialogueItem[] => {
  return useMemo(() => {
    return indexes
...
export const useTextSegments = (screenplay: DialogueItem[], index: number): string[] => {
  return useMemo(() => {
    if (index >= 0 && index < screenplay.length) {
